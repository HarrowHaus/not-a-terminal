 ...
 *
 * Shards are built by the pipeline (scripts/) and live in public/indexes/ as
 * {category}.json + {category}.json.gz. Each template carries its 384-dim
...
import { bulkInsertTemplates, isShardLoaded, markShardLoaded } from './db'
import type { ShardTemplateRow } from './db'
...
async function fetchShard(category: string): Promise<ShardJson | null> {
  ...
  try {
    const res = await fetch(`/indexes/${category}.json.gz`)
  ...
  try {
    const res = await fetch(`/indexes/${category}.json`)
